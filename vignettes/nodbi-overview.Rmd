...
This is another powerful function because  `value` can come from a JSON string, a data frame, a list or a file with `NDJSON` lines, and if `value` includes already `_id`'s, these are used to identify the documents that should be updated. The function uses specific and fast `SQL` functions for most datsabases. 
