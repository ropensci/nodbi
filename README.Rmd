...
across all database backends. \*Only simple queries (and updates, e.g. equality for a single field) supported for Elasticsearch at the moment. \*\*Only root fields can be specified for CouchDB, whereas fields with subitems (in dot notation) can be specified for Elasticsearch, MongoDB, SQLite and PostgreSQL.
