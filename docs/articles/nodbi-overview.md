...
[`docdb_query`](https://docs.ropensci.org/nodbi/reference/docdb_query.md)`(``src``, ``key``, query ``=`` ``'{"_id": {"$regex": "^[0-9]"}}'``, listfields ``=`` ``TRUE``)`` ``#> [1] "about" "age" "balance" "email" "eyeColor" `` ``#> [6] "friends" "friends.id" "friends.name" "isActive" "name" `` ``#> [11] "registered" "tags"`
